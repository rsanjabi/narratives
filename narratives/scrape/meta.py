...
class Meta(Page):
    def __init__(self, tag: str, from_top: bool = True):
        self.base_url = (
            f"https://archiveofourown.org/tags/"
    ...
    def _pages(self) -> Generator[Tuple[BeautifulSoup, str], None, None]:
        ...
        try:
            ...
            self.logger.error(
                f"Last scraped value ({self.last})" f" in .meta is not a number"
            )
            raise ValueError
        ...
        try:
            ...
            raise ConnectionError(
                f"Error connecting to: {self.base_url}\n"
        ...
        while errors < cfg.MAX_ERRORS and page_num <= max_pages:
            try:
                ...
                self.logger.error(
                    f"PAGE: {url} 404 Error. Skipping this work."
                ...
                errors += 1
                self.logger.error(
                    f"PAGE: {url} Not found. " f"{cfg.MAX_ERRORS-errors} attempts left."
                )
    ...
    def _page_elements(self, page: BeautifulSoup) -> Generator[MetaJson, None, None]:
    ...
    def _total_pages(self) -> int:
        ...
        for attempts in range(cfg.MAX_ERRORS):
            try:
                ...
                self.logger.info(
                    f"Attempting to scrape up to " f"{str(max_pages)} pages."
                )
                return max_pages
                ...
                self.logger.error(
                    f"Base URL: {self.base_url} Not found. "
    ...
    def _get_tag_info(self, category: str, meta: BeautifulSoup) -> Optional[List[str]]:
        """ Find relationships, characters, and freeforms tags."""
        try:
    ...
    def _get_series(self, work: BeautifulSoup) -> Tuple[Optional[str], Optional[str]]:
